...
class ToolHeadCommandHelper:
    ...
    def cmd_SET_VELOCITY_LIMIT(self, gcmd):
        eventtime = self.toolhead.reactor.monotonic()
        #logging.info("\033[33m into cmd_SET_VELOCITY_LIMIT\033[0m")
        # logging.info(eventtime)
        # logging.info(self.toolhead.print_stats.get_status(eventtime)['print_duration'])
        if self.toolhead.print_stats.get_status(eventtime)['print_duration'] > 0.0:
            #logging.info("\033[33m into diy cmd_SET_VELOCITY_LIMIT\033[0m")
            if self.toolhead.print_mode == 0:
                self.toolhead.conf_max_velocity = self.toolhead.max_velocity_normal
                self.toolhead.conf_max_accel = self.toolhead.max_accel_normal
                self.toolhead.conf_max_accel_to_decel = self.toolhead.max_accel_to_decel_normal
                self.toolhead.get_conf_max_velocity_success = True
                self.toolhead.get_conf_max_accel_success = True
                self.toolhead.get_conf_max_accel_to_decel_success = True
            elif self.toolhead.print_mode == 1:
                self.toolhead.conf_max_velocity = self.toolhead.max_velocity_silent
                self.toolhead.conf_max_accel = self.toolhead.max_accel_silent
                self.toolhead.conf_max_accel_to_decel = self.toolhead.max_accel_to_decel_silent
                self.toolhead.get_conf_max_velocity_success = True
                self.toolhead.get_conf_max_accel_success = True
                self.toolhead.get_conf_max_accel_to_decel_success = True
            elif self.toolhead.print_mode == 2:
                self.toolhead.conf_max_velocity = self.toolhead.max_velocity_berserk
                self.toolhead.conf_max_accel = self.toolhead.max_accel_berserk
                self.toolhead.conf_max_accel_to_decel = self.toolhead.max_accel_to_decel_berserk
                self.toolhead.get_conf_max_velocity_success = True
                self.toolhead.get_conf_max_accel_success = True
                self.toolhead.get_conf_max_accel_to_decel_success = True
        else:
            self.toolhead.get_conf_max_velocity_success = False
            self.toolhead.get_conf_max_accel_success = False
            self.toolhead.get_conf_max_accel_to_decel_success = False

        max_velocity = gcmd.get_float('VELOCITY', None, above=0.)
        if max_velocity is not None:
            if self.toolhead.get_conf_max_velocity_success:
                if max_velocity > self.toolhead.conf_max_velocity:
                    max_velocity = self.toolhead.conf_max_velocity
        max_accel = gcmd.get_float('ACCEL', None, above=0.)
        if max_accel is not None:
            if self.toolhead.get_conf_max_accel_success:
                if max_accel > self.toolhead.conf_max_accel:
                    max_accel = self.toolhead.conf_max_accel
        ...
        if get_conf_print_mode:
            if print_mode == set_print_mode:
                ...
                if set_print_mode == 0:
                    ...
                    self.toolhead.print_mode = 0
                elif set_print_mode == 11:
                    self.toolhead.print_mode = 1
                elif set_print_mode == 12:
                    self.toolhead.print_mode = 2
                else :
    ...
    def cmd_M204(self, gcmd):
        ...
        if accel is None:
            ...
            accel = min(p, t)
        if self.toolhead.get_conf_max_accel_success:
            if accel > self.toolhead.conf_max_accel:
                accel = self.toolhead.conf_max_accel
    ...
    def cmd_FAN_MODE(self, gcmd):
        ...
        if self.toolhead.fan_mode == set_fan_mode:
            self.gcode.respond_info("Nothing to do: Fan mode is %d" % (set_fan_mode))
        else:
            self.toolhead.fan_mode = set_fan_mode
